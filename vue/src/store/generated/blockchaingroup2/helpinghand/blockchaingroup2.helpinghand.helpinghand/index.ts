	...
	actions: {
		...
		async QueryCompletion({ commit, rootGetters, getters }, { options: { subscribe = false , all = false}, params: {...key}, query=null }) {
			try {
				...
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryCompletion', payload: { options: { all }, params: {...key},query }})
		...
		async QueryCompletionAll({ commit, rootGetters, getters }, { options: { subscribe = false , all = false}, params: {...key}, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.nextKey!=null) {
					let next_values=(await (await initQueryClient(rootGetters)).queryCompletionAll({...query, 'pagination.key':(<any> value).pagination.nextKey})).data
				...
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryCompletionAll', payload: { options: { all }, params: {...key},query }})
